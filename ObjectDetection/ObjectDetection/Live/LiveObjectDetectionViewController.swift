...
class LiveObjectDetectionViewController: ViewController {
    ...
    override func viewDidLoad() {
        ...
        cameraController.videoCaptureCompletionBlock = { [weak self] buffer, error in
            ...
            guard let outputs = self?.inferencer.detectmodule.detect(image: &pixelBuffer) else {
                return
            }
            ...
            DispatchQueue.main.async {
                ...
                var dets : [[Double]] = []
                for (id, pred) in nmsPredictions.enumerated(){
                    let rect = pred.rect
                    let x1 = Double(rect.origin.x - rect.size.width / 2)
                    let y1 = Double(rect.origin.y - rect.size.height / 2)
                    let x2 = Double(rect.origin.x + rect.size.width / 2)
                    let y2 = Double(rect.origin.y + rect.size.height / 2)
                    let score = Double(pred.score)
                    let det = [x1, y1, x2, y2, score]
                    dets.append(det)
                }

                strongSelf.sort_tracker.update(dets: dets)
